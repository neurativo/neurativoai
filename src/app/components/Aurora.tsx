...
void main() {
  ...
  // Keep horizontal position fixed; animate only vertically over time
  float height = snoise(vec2(uv.x * 2.0, uTime * 0.20)) * 0.5 * uAmplitude;
  height = exp(height);
...
export default function Aurora(props: AuroraProps) {
	...
	useEffect(() => {
		...
		const update = (t: number) => {
			...
			(program!.uniforms.uTime as any).value = (time as number) * (speed as number) * 0.1;
	...
	return <div ref={ctnDom} className="aurora-container fixed inset-0 -z-10 pointer-events-none" />;
}
